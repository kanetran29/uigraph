...
- **report_observation** `{from, to, event, outcome, effect?, proposalId?, evidence?, reportedBy?, screenshot?}` — record a runtime attempt. `confirmed` REQUIRES proof: `evidence` ({kind:'url-change',startUrl,landedUrl} | {kind:'url-assert',url} | {kind:'dialog'} | {kind:'screenshot',path} — the file must exist) plus `reportedBy` ('runner'|'agent'); without valid proof the call returns `{error}` and records nothing. An accepted confirmation folds into a `runtime` edge (guard and modality are preserved on guarded edges) AND reconciles the linked proposal to `confirmed`; `refuted` needs no proof, adds no edge, and reconciles to `rejected`. Returns the entry + `reconciled[]`.
- **propose** `{proposals: [{kind, category, screen, title, rationale, confidence, evidenced?, event?, to?, guard?, effect?}]}` — submit batched Tier-2 hypotheses. Be greedy: propose every plausible interaction/state the extractor cannot see (modals, disclosure, infinite scroll, keyboard, error/empty states); a wrong proposal costs nothing — it is quarantined and can never enter the proven graph. Edge proposals need an `event`; `to` is a node id or `"<modal>"`. Returns minted ids + per-item rejections with reasons.
