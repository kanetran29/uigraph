...
| g21 | dialog confirm navigates, cancel closes | `ProductsIndex.tsx` | may-edge `/products` -> `/checkout` from the confirm button | may | Recalibrated: the confirm button only exists while the modal is open (state-gated render), and uigraph's calibration caps state-gated controls at `may` by design — promoting it risks phantom musts. The transition is real; runtime verify promotes it to witnessed. |
